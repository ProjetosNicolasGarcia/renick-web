  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] flex flex-col md:flex-row pt-8 md:pt-16 px-4 md:px-16 font-poppins max-w-6xl mx-auto gap-4 md:gap-8">
      ...
      <nav className="w-full md:w-64 flex flex-col gap-4 mb-8 md:mb-0">
        ...
        <button onClick={() => { setActiveTab('dados'); resetState(); }} className={`cursor-pointer w-fit text-left font-bold text-[20px] uppercase transition-colors ${activeTab === 'dados' ? 'text-[#1E45FB]' : 'text-[#0A0A0A] hover:text-[#1E45FB]'}`}>DADOS DA CONTA</button>
        <button onClick={() => { setActiveTab('pedidos'); resetState(); }} className={`cursor-pointer w-fit text-left font-bold text-[20px] uppercase transition-colors ${activeTab === 'pedidos' ? 'text-[#1E45FB]' : 'text-[#0A0A0A] hover:text-[#1E45FB]'}`}>PEDIDOS</button>
        
        <Link to="/addresses" className="cursor-pointer w-fit text-left font-bold text-[20px] uppercase transition-colors text-[#0A0A0A] hover:text-[#1E45FB]">
          ENDEREÇOS
        </Link>
        
        <button onClick={() => { setActiveTab('favoritos'); resetState(); }} className={`cursor-pointer w-fit text-left font-bold text-[20px] uppercase transition-colors ${activeTab === 'favoritos' ? 'text-[#1E45FB]' : 'text-[#0A0A0A] hover:text-[#1E45FB]'}`}>FAVORITOS</button>
      ...
      <div className="flex-1 w-full max-w-[480px]">
        {activeTab === 'dados' && (
          <div className="flex flex-col gap-4">
            ...
            <h2 className="font-suez text-[32px] md:text-[40px] text-[#1E45FB] uppercase mb-4">Dados da Conta</h2>
        ...
        {activeTab !== 'dados' && (
          <div>
            {/* Título de seção atualizado com as diretrizes de H1 */}
            <h2 className="font-suez text-[32px] md:text-[40px] text-[#1E45FB] uppercase mb-4">
              {activeTab === 'pedidos' ? 'PEDIDOS' : 'FAVORITOS'}
            </h2>
